...
func main() {
	...
	//cartridge, err := LoadCartridge("roms/test.gb")
	...
	rl.InitWindow(1500, 500, "raylib [core] example - basic window")
	...
	for !rl.WindowShouldClose() {
		...
		//logger.Debug(fmt.Sprintf("DEBUG: LCDC: %08b  LY %d \n", lcdc, ly))
		drawTileMap(cpu, 400, 0)

		//scale := 4
		//for y := 0; y < screenHeight; y++ {
		//	for x := 0; x < screenWidth; x++ {
		//ClearBackground		color := colors[cpu.graphics.pixelBuffer[y][x]]
